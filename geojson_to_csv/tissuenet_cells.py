...
TISSUENET_CELLS_JSON_DIR = "/home/carlos/Desktop/RESULTS_QUPATH/tissuenet_cells_save"
TN_CELLS_MASK_DIR = ...
RES_FILE = "/home/carlos/eclipse-workspace-test/scripts/res_qupath_second/tissuenet_cells.csv"
os.makedirs(os.path.dirname(RES_FILE), exist_ok=True)

all_files = os.listdir(TN_CELLS_MASK_DIR)
all_files.sort()
...
f_names = []
mask_arr = np.load(os.path.join(TN_CELLS_MASK_DIR, "test.npz"))["y"]
for row_idx in range(mask_arr.shape[0]):
    print(row_idx)
    gf_name = f"im_{row_idx}.png"
    f_names.append(gf_name)
    mask = mask_arr[row_idx, :, :, 0]
    ...
    for i_model, model in enumerate(models):
        ...
        gm_path = os.path.join(TISSUENET_CELLS_JSON_DIR, gm_name)
