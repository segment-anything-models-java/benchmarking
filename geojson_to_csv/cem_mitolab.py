...
CEM_JSON_DIR = "/home/carlos/Desktop/RESULTS_QUPATH/neurips_save"
CEM_MITOLAB_DIR = ...
MASK_FOLDER = "masks"
RES_FILE = "/home/carlos/eclipse-workspace-test/scripts/res_qupath_second/cem_mitolab.csv"
...
ARBITRARILY_LARGE = 20_000


scores_mat = np.zeros((ARBITRARILY_LARGE, len(models) * len(promtp_types)), dtype="float64")
f_names = []
bn_counter = -1

available_files = "/path/to/reduced/mitoem" ## PATH to the folder where there are only 1000 mitoem images
for ff in (all_files):
    all_file_2 = os.listdir(os.path.join(CEM_MITOLAB_DIR, ff, MASK_FOLDER))
    all_file_2.sort()
    for ff2 in all_file_2:
        gf_name = ff + "_" + ff2 + ".tiff"
        if gf_name not in available_files:
            continue
        bn_counter += 1
        print(bn_counter)
        ...
        for i_model, model in enumerate(models):
            ...
            gm_path = os.path.join(CEM_JSON_DIR, gm_name)
            ...
            for k, prompt_name in enumerate(promtp_types):
                ...
                scores_mat[bn_counter, i_model * len(promtp_types) + k] = mean_nonzero
                #print(row_idx, model, prompt_name, mean_nonzero)

scores_mat = scores_mat[: bn_counter + 1]
import polars as pl
cols = []
