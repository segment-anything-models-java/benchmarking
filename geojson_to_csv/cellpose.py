...
CELLPOSE_JSON_DIR = "/home/carlos/Desktop/RESULTS_QUPATH/cellpose_save"
#CELLPOSE_JSON_DIR = "/home/carlos/Videos/QuPath-v0.6.0-Linux/cellpose/results/cellpose_save"
CELLPOSE_MASK_DIR = "/home/carlos/Pictures/samj_rebuttal/cellpose/test"
RES_FILE = "/home/carlos/eclipse-workspace-test/scripts/res_qupath_second/cellpose.csv"
...
all_files = os.listdir(CELLPOSE_MASK_DIR)
all_files.sort()
mask_files = [f for f in all_files if "mask" in f]
mask_files.sort()

scores_mat = np.zeros((len(mask_files), len(models) * len(promtp_types)), dtype="float64")
f_names = []

for row_idx, mask_name in enumerate(mask_files):
    gf_name = mask_name[:3] + "_img.png"
    f_names.append(gf_name)

    mask = np.array(Image.open(os.path.join(CELLPOSE_MASK_DIR, mask_name)))
    ...
    for i_model, model in enumerate(models):
        ...
        gm_path = os.path.join(CELLPOSE_JSON_DIR, gm_name)
