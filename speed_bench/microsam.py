...
def call(image: im_layer, model_type: str, skip_validate=False):
    ...
    def compute_image_embedding():
        ...
        state.initialize_predictor(
            ...
            device="cpu", checkpoint_path=None, tile_shape=tile_shape, halo=halo,
...
for cc, ff in enumerate(all_files[:2]):
    print(cc)
    f_names.append(ff)
    ...
    for j, model_type in enumerate(model_types):
        ...
        end_time = time()
        print(f"Loading time for {model_type}: {end_time - start_time}")
        ...
        for ib, bb in enumerate(bbox_prompts):
            ...
            end_time = time()
            tot_time += (end_time - start_time)
        print(f"Prompting time for {model_type}: {tot_time / (len(bbox_prompts) + len(point_prompts))}")
