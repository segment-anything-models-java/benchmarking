...
scores_mat = np.zeros((n_files, len(model_types) * len(promtp_types)), dtype="float64")
...
for cc in range(n_files):
    ...
    if (im != im.astype("uint8")).any():
        im[:, :, 0] = (255 * ((im[:, :, 0] - im[:, :, 0].min()) / (im[:, :, 0].max() - im[:, :, 0].min())))
        im[:, :, 1] = (255 * ((im[:, :, 1] - im[:, :, 1].min()) / (im[:, :, 1].max() - im[:, :, 1].min())))
    im = im.astype("uint8")
    img = Image.fromarray(im)
    path = os.path.join(TN_DIR, REAL_FOLDER, f"im_{cc}.png")
    img.save(path)
    ...
    for i in range(1, mask.max() + 1):
        ...
        # points.append([pps])
        points.append(pps)
    np.save(os.path.join(POINT_PROMPTS, f"im_{cc}.npy"), np.array(points))
